...
- Added deployment hardening for Render with lightweight backend deploy dependencies, Python 3.11 runtime pinning, and CORS support for the deployed frontend.
- Phase 26 deployment completed with Render backend, Vercel frontend, production API environment wiring, and live endpoint verification.

## Current Module

- Phase 27: Monitoring and logging.
...
- Deployed React frontend is configured with the Render API base URL through Vercel environment variables.
...
- Use a separate `requirements.deploy.txt` for Render so the fallback production demo avoids heavy optional AI/dev packages during web-service builds.
- Allow browser CORS for the deployed demo frontend; tighten allowed origins before a real authenticated production launch.
- Deploy frontend on Vercel and backend on Render as separate services; connect them through `VITE_API_BASE_URL` instead of hardcoding backend URLs in source.
...
Implement Phase 27: Monitoring and logging.

The next module should add:

- structured backend logs
- frontend error-state logging notes
- deployment health-check documentation
