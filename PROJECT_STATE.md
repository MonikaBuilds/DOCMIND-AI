...
- Deployed React frontend must be configured with `VITE_API_BASE_URL=https://<render-backend-url>/api/v1` after the Render backend succeeds.
...
- Vercel frontend environment update with the Render API URL
- production smoke test for upload, prepare, and chat
